...
OUTDIR=/pub/$USER/test_task_dir_read_l1/$idx
mkdir -p  $OUTDIR
mkdir -p  $OUTDIR/out
...
for i in `seq 0 9`;
    do
        echo 'python /data/users/maityb/Modified/PID/video.py write_ber=0.0 read_ber='$nbr' index='$idx$i' frameName=marymax_8 >/dev/null 2>&1' >>  $OUTDIR/$FILENAME
    done

echo "# Running on `hostname`" >> $OUTDIR/$FILENAME
cp config_l1d.ini $OUTDIR/config.ini
